...
class RobotDebug(GeneralNavigator):
    def __init__(self, agent, details):
        ...
        #step_delay=5 #TODO: remove this once testing completed
        default = rospy.get_param(param, step_delay)
        #rospy.set_param(param, default)
...
publish_path()
    ...
    def publish_node_tf(self):
        logmsg(category='vr_rob', id=self.agent.agent_id, msg='        | a) Pub Node TF')
    ...
    def publish_node_pose(self):
        logmsg(category='vr_rob', id=self.agent.agent_id, msg='        | b) Pub Node Pose')
        pos, ori = self.agent.map_handler.get_node_tf(self.execpolicy_goal.route.source[0])
        pose = Pose(Point(x=pos[0],y=pos[1],z=pos[2]), Quaternion(x=ori[0],y=ori[1],z=ori[2],w=ori[3]))
        self.pose_publisher.publish(pose)
    def publish_edge_pose(self):
        logmsg(category='vr_rob', id=self.agent.agent_id, msg='        | b) Pub Edge Pose')
        n1, n2 = self.execpolicy_goal.route.edge_id[0].split('_')
        pos1, ori = self.agent.map_handler.get_node_tf(n1)
        pos2, _   = self.agent.map_handler.get_node_tf(n2)
    ...
    def publish_node_name(self):
        logmsg(category='vr_rob', id=self.agent.agent_id, msg='        | c) Pub Node Name')
    ...
    def publish_path(self):
        logmsg(category='vr_rob', id=self.agent.agent_id, msg='        | d) Pub Path')
        route = Path()
...
#    def set_execpolicy_goal(self, exec_policy_goal):
#        # Publish full path to rviz
#        self.rviz_path = self.generate_path(exec_polify_goal)
#        self.rviz_full_route_publisher.publish(rviz_path)
#        self.update_robot_location(path=rviz_path)
#
#        # Publish exec policy to cycle
#        self.execpolicy_goal = goal
#        self.publish_exec_policy_goal(goal)
#        self.total_routes += 1


#    def generate_path(exec_policy_goal):
#        source = exec_policy_goal.route.source or []
#        edge_id = exec_policy_goal.route.edge_id or []
#
#        # Generate rviz-compatible route
#        route = Path()
#        route.header.frame_id = "map"
#        if source:
